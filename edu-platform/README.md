...
- **«Математика: с нуля до ЕГЭ»** (`seed:math`): 8 модулей, 24 урока, 16 домашек,
  ~390 блоков контента, ~140 тест-вопросов. Объяснения «как для чайника», формулы
  (KaTeX), наглядные SVG-схемы, жизненные примеры. Темы: натуральные числа → дроби
  → отрицательные числа → уравнения → проценты/степени/корни → функции и графики →
  квадратные уравнения → тригонометрия и подготовка к ЕГЭ.
- **«ЕГЭ по математике: пробные варианты»** (`seed:ege`): 8 пробных вариантов
  (базовые, вероятность/статистика, производная/стереометрия, ловушки и др.) с
  полными пошаговыми разборами, схемами, ответами и **авто-проверяемыми тестами**
  (ученик проходит тест и сразу получает балл — 64 тест-вопроса).

Редактор урока (стиль Jupyter) поддерживает режим **«Предпросмотр»** и живой
**предпросмотр в каждой ячейке** — формулы и схемы видно прямо при наборе.
Веб — **PWA**: устанавливается на телефон и работает офлайн (кэш-оболочка).
